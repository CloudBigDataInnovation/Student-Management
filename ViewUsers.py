...
def viewUser(): 
    ...
    headingLabel = Label(headingFrame2, text="VIEW USERS", fg='black')
    headingLabel.place(relx=0.25,rely=0.15, relwidth=0.5, relheight=0.5)   
    
    y = 0.25
    
    Label(labelFrame, text="%-20s%-30s%-20s"%('User #','User_Name','Role'),bg='black',fg='white').place(relx=0.07,rely=0.1)
    Label(labelFrame, text="----------------------------------------------------------------------------",bg='black',fg='white').place(relx=0.05,rely=0.2)
    getCourses = "select user_number, user_name, user_role from "+userTable
    try:
        cur.execute(getCourses)
        con.commit()
        for i in cur:
            Label(labelFrame, text="%-20s%-30s%-20s"%(i[0],i[1],i[2]),bg='black',fg='white').place(relx=0.07,rely=y)
            y += 0.1
    except:
