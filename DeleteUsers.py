...
def deleteUser():
    ...
    deleteSql = "delete from "+userTable+" where user_number = '"+user_number+"'"
    try:
        cur.execute(deleteSql)
        con.commit()
        messagebox.showinfo("Successful","Deleted User!")
    except:
        messagebox.showinfo("Check Credentials","Please check User ID")
...
def userDelete(): 
    ...
    headingLabel = Label(headingFrame2, text="DELETE USER", fg='black')
    headingLabel.place(relx=0.25,rely=0.15, relwidth=0.5, relheight=0.5)   
        
    # Book ID to Delete
    lb2 = Label(labelFrame,text="USER Number : ", bg='black', fg='white')
    ...
    #Submit Button
    SubmitBtn = Button(root,text="SUBMIT",bg='#d1ccc0', fg='black',command=deleteUser)
