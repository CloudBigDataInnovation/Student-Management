...
def search():
    ...
    Label(labelFrame, text="%-10s%-30s%-20s%-30s%-20s"%('Student Number','Name','Course Number','Course Name','Grade'),bg='black',fg='white').place(relx=0.07,rely=0.1)
    Label(labelFrame, text="-"*120,bg='black',fg='white').place(relx=0.05,rely=0.2)
    
    searchSql = "select * from "+gradeTable+" where subject = '"+sub+"'"
    try:
...
def viewGrade(): 
    ...
    headingLabel = Label(headingFrame2, text="VIEW GRADE", fg='black')
